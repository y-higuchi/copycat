      ...
      client.connect().whenComplete((result1, error1) -> {
        if (error1 != null) {
          ...
          client.poll(new PollRequest(context.nextCorrelationId(), context.currentTerm(), context.clusterManager()
            ...
            if (!complete.get()) {
              if (error2 != null) {
                LOGGER.warn("{} - Polling {} failed with Exception: {}", context.clusterManager().localNode(), node.member(), error2.getMessage());
                LOGGER.debug("{} - Polling {} failed with Exception", context.clusterManager().localNode(), node.member(), error2);
                quorum.fail();
